...
# summary
print(np.mean(F1_select_exact))
print(np.mean(coverage_exact))
print(np.mean(length_exact))
results = pd.DataFrame(np.column_stack((F1_select_exact, F1_infere_exact, coverage_exact, length_exact)),
                       columns = ['F1_scores_exact', 'F1_infere_exact', 'coverage_exact', 'length_exact'])
results.to_csv('results_high_rdm6.csv', index=False)
